...
19. La fenêtre principale est exclue de la capture d'écran (`SetWindowDisplayAffinity` / `WDA_EXCLUDEFROMCAPTURE`) dès qu'au moins un coffre est déverrouillé : un outil de capture, un enregistreur d'écran ou un partage d'écran (visioconférence, RDP) l'affiche noire ou l'ignore, alors qu'elle reste normalement visible pour l'utilisateur. Retirée dès que tous les coffres sont verrouillés, y compris par inactivité ou détection de mise en veille.
